    ...
    throws ServletException, IOException {
        ...
        if (idRaw != null) {
            try {
                ...
                List<OrderStuff> gearList = dao.getGearByOrderId(id);
                
                if (order != null) {
                    request.setAttribute("order", order);
                    request.setAttribute("gearList", gearList);
                    request.getRequestDispatcher("/templates/admin/order-detail.jsp").forward(request, response);
                } else {
                    response.sendRedirect(request.getContextPath() +"/AdminOrder"); // Quay lại danh sách nếu ko tìm thấy
                }
